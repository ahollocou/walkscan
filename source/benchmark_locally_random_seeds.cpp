...
static void PrintBenchmarkLocallyRandomSeedsUsage() {
    ...
    printf("\t-d [distance to ground-truth] : Specifies the maximum distance from the ground truth communities where seed nodes can be picked.\n");
...
int RunBenchmarkLocallyRandomSeeds(int argc, char ** argv,
    ...
    bool distanceToGroundTruthSet = false;
    ...
    uint32_t distanceToGroundTruth = 0;
    ...
    for(int i = 1; i < argc; i++) {
        ...
        CHECK_ARGUMENT_INT(i, "-d", distanceToGroundTruth, distanceToGroundTruthSet)
    ...
    if (!outputFileNameSet) {
        ...
        PrintBenchmarkLocallyRandomSeedsUsage();
        return 1;
    }

    if (!distanceToGroundTruthSet) {
        printf("Distance to ground-truth communities not set\n");
        PrintBenchmarkLocallyRandomSeedsUsage();
        return 1;
    }
    ...
    if (seedSetFileNameSet) {
        ...
        PickRandomSeedsNearGroundTruth(nodeNeighbors, groundTruthCommunities, seeds, proportion, distanceToGroundTruth);
        printf("Nb of seed sets: %lu\n", seeds.size());
